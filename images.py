...
def draw_graph(filename: str, nodes: Dict[str, Node], edges: List[Edge]) -> None:
    img = __create_baseimage(SIZE, BG_COLOR, nodes, edges)
    img.save(filename)


def draw_path(filename: str, nodes: Dict[str, Node], edges: List[Edge], path: List[Node]) -> None:
